namespace ChainTicker.Transport.Rest
{
    public class RestQueryUri
    {
        ...
        public RestQueryUri(string serviceBaseUri, string path)
        {
            _serviceBaseUri = serviceBaseUri.TrimEnd('/');
            _path = path.TrimStart('/');
        }

        public RestQueryUri(string serviceBaseUri, string path, string queryString) : this(serviceBaseUri, path)
        {
